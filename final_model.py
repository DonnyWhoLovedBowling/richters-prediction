import mlflow
from train_by_features import train_by_features, create_submission, train_gbc_by_features, train_bag_by_features
...
mlflow.set_experiment("Richters prediction - final model")
f1, model = train_bag_by_features(top_features)
...
for of in other_features:
    if of in ['height_percentage']:
        continue
    training_features = list(top_features) + [of]
    f1, model = train_by_features(training_features)
    ...
    with mlflow.start_run():
        ...
        model_info = mlflow.sklearn.log_model(
            sk_model=model,
            artifact_path="final_model_top_5_with_1_extra_random_forest",
            registered_model_name="final_model_top_5_with_1_extra_random_forest",
        )
